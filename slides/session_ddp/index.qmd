...
- A __Markov Chain__ with values in $S$ and with transitions given by a stochastic matrix $P\in R^n\times R^n$ identfies a __stochastic process__ $(X_t)_{t\geq 0}$ such that $$P_{ij} = Prob(X_{t+1}=s_j|X_t=s_i)$$
...
__Irreducible__: all states can be reached with positive probability from any initial state.
...
%%| fig-height: 5
flowchart LR
  %% A(Poor) --> |0| B(Middle Class)
  B(Middle Class) --> |0.11| A(Poor)
  B --> |0.2| C(Rich)
  C --> |0.1| B
  C --> |0.1| A
```
...
```{.julia code-line-numbers="[1-2|3-6|7-9|10-12|13-14|15-17]"}
...
- <!-- .element class="fragment" --> <b>Environment</b>
  - states: $s \in S$
  - actions: $x \in X(s)$
...
- Horizon is infinite:
$$V(s) =  \max E_0 \sum_{t=0}^{\infty} \delta^t r(s_t, x_t) $$
- Intuition:
...
- Taking continuation value as given we can certainly improve the value in every state $\tilde{V}$ by choosing $\tilde{x}()$ so as to maximize
$$\tilde{V}(s; \tilde{x}(), x()) =  r(s, \tilde{x}(s)) + \delta \sum_{s'} \pi(s'|s,\tilde{x}(s) )V(s'; x()) $$
...
- The function $G$ is known as the __Bellman operator__: $$G: V \rightarrow \max_{y(s)} r(s, y(s)) + \delta \sum_{s^{\prime}\in S} \pi(s^{\prime}| s, y(s)) V(s^{\prime})$$ 
...
$$V_a^U(w) = U(\underline{c}) + \begin{cases} \beta {V}^E_a(w) & \text{if $a(w)$ is true} \\ \beta  E_{w'}\left[ {V}^U_a(w^{\prime}) \right]  & \text{if $a(w)$ is false} \end{cases}$$
$$V_a^E(w) = U(w) +  (1-\lambda) \beta {V}^E_a(w) +  \lambda \beta E_{w'}\left[{V}^U_a(w^{\prime}) \right] $$
